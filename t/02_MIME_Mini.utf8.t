...
{
	plan tests => 6 + ($display_enc eq 'UTF-8' ? 1 : 0);
	...
	my $h6 = 'text with Φne non-ascii non-latin1 char for utf8 and qp';
...
Header5: text with Q-encoding special =?iso-8859-1?q?=F6=3F=3D=5Fchars?= for
 qp
Header6: text with =?utf-8?q?=CE=A6ne?= non-ascii non-latin1 char for utf8 and
 qp
...
', 'newheader: long encoded headers';
	...
	cmp_deeply [display_encode header($m, 'header6')], [display_encode $h6], 'newheader: one non-ascii non-latin1, utf8 qp' if $display_enc eq 'UTF-8';
};
